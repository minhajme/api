...
import { run_omuk_times, generate_apikey, parse_api_url, init_test_system, 
assert_true, assert_equal, assert_json, assert_substring } from "./utils.mjs";


const test_set_cell_value = () => {
...
const test_ratelimit_real = async () => {
	const close_test_system = await init_test_system(
        { seed: () => set_cell_value('A2', ['testapikey1', '1', '6'], { append:true }) } );
    const [apikey, enabled, rate] = run_omuk_times(
        cellname => get_cell_value(cellname), ["A2", "B2", "C2"]);
    ...
    run_omuk_times(() => {
        test("", page => {
            await page.goto(url);
            assert_equal(page.waitForResponse(resp => resp.url===url).status(), 
                            apisettings.http_code.succeed);
            assert_json(await page.content());
        });
	}, rate);
    run_omuk_times(test('', page => {
        await page.goto(url);
...
const test_apiserver_urlvr_requirement = async () => {
    const close_test_system = init_test_system({ seed: () => {
