...
const settings = require('./apisettings.mjs');
...
const server = http.createServer((request, response) => {
    ...
    if (request.url === '/favicon.ico') {
        ...
        const [client_apikey, stocksymbols] = \
		request.url.replace('/vr/', '').replace(/^\/+|\/+$/g, '').split('/');
        const { rate as rate_mn } = get_data_rows().filter(row => row.apikey===client_apikey);
        ...
        assert_truthy(limiter_monthly.available(), 
                `apilimitexpire: rate per hr limit ${rate} exceeded!`);

        https.get(
            'https://api.gurufocus.com/public/user/' + settings['GURU_API_KEY'] 
            + '/stock/' + settings.request.stocksymbols + '/financials', res => {
            limiter.log();
            let data = [];
            res.on('data', chunk => {
                data.push(chunk);
...
server.listen(settings.apiurlport, settings.apiurlhost, () => {
    console.log(`Server running at http://${settings.apiurlhost}:${settings.apiurlport}/`);
});
